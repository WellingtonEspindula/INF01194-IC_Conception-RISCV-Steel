...
##Generate power ring with 0.25um spacing (between metal lines), 0.5um width and 1.5um offset from the core. Use M2 for horizontal and M3 for vertical
add_rings -layer {top M4 bottom M4 left M3 right M3} -jog_distance 2.5 -nets {gnd vdd} -threshold 2.5 -width 0.44 -spacing 0.46 -offset 0.315
#-spacing 0.25 -width 0.5 -offset 1.5
...
#route_special -connect {block_pin pad_pin pad_ring core_pin floating_stripe} -layer_change_range { M1 METTP } -block_pin_target nearest_target -pad_pin_port_connect {all_port one_geom} -pad_pin_target nearest_target -core_pin_target first_after_row_end -floating_stripe_target {block_ring pad_ring ring stripe ring_pin block_pin followpin} -allow_jogging 1 -crossover_via_layer_range { M1 METTP } -nets { gnd vdd } -allow_layer_change 1 -block_pin use_lef -target_via_layer_range { M1 METTP }

##Add well taps
add_well_taps -cell FEED1 -cell_interval 20 -fixed_gap -prefix WELLTAP -in_row_offset 8.0

##Add power stripes
add_stripes -block_ring_top_layer_limit M3 -max_same_layer_jog_length 6 -pad_core_ring_bottom_layer_limit M1 -set_to_set_distance 25 -pad_core_ring_top_layer_limit M4 -spacing 6 -merge_stripes_value 2.5 -layer M3 -block_ring_bottom_layer_limit M1 -width 0.5 -nets {gnd vdd}
